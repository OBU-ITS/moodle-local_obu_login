...
/**
 * OBU Login - non-standard login page (ignores any alternative login URL)
 *
 * @package    local_obu_login
 * @author     Martin Dougiamas.  Adopted and adapted (slightly) by Peter Welham (OBU)
 * @copyright  1999 onwards Martin Dougiamas http://dougiamas.com and 2015, Oxford Brookes University
...
$PAGE->set_url("$CFG->httpswwwroot/local/obu_login/index.php");
...
if (empty($SESSION->wantsurl)) {
    $SESSION->wantsurl = (array_key_exists('HTTP_REFERER',$_SERVER) &&
                          ...
                          $_SERVER["HTTP_REFERER"] != $CFG->wwwroot.'/' &&
                          $_SERVER["HTTP_REFERER"] != $CFG->httpswwwroot.'/local/obu_login/' &&
                          strpos($_SERVER["HTTP_REFERER"], $CFG->httpswwwroot.'/local/obu_login/?') !== 0 &&
                          strpos($_SERVER["HTTP_REFERER"], $CFG->httpswwwroot.'/local/obu_login/index.php') !== 0) // There might be some extra params such as ?lang=.
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new moodle_url('/local/obu_login/index.php'));
}
